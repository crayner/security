...
class Authorisation extends AuthorizationChecker implements ContainerAwareInterface
{
	 ...
	 * @return bool
	 */
	private function tablesCreated(EntityManager $om)
	{
		if (class_exists(Failure::class))
		{
			$metaData = $om->getClassMetadata(Failure::class);
			$schema   = $om->getConnection()->getSchemaManager();
